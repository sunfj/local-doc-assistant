...
## 一、问题的起点：隐私敏感文档的 AI 困境

想象一个场景：你收到一份采购合同的扫描件，需要立刻找到其中的违约金条款。你想用 AI 帮忙——但合同涉及商业价格和客户信息，**上传到云端不放心**。

这不是假设。法律合同、病历扫描件、身份证照片、内部审计报告——这些文档的共同特点是：**用户迫切需要 AI 帮忙提取信息，但绝不允许数据离开本机**。
...
当 Skill 在本地 Ollama 上跑通后，我们尝试把它部署到 QwenPaw上。
