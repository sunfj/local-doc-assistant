...
> Skill 发布地址：modelscope.cn/skills （TODO：发布后补链接）
> 关键词：AI PC、OpenVINO、INT4 量化、BGE Embedding、FAISS、Qwen3.6、Tool Calling、隐私计算
...
| 组件 | 选型 | 关键理由 |
|---|---|---|
| Agent 大脑 | Qwen3.6-35B-A3B (Ollama) | 比赛验证基准；MoE 激活仅 3B；原生 Tool Calling + Thinking |
...
| 推理框架 | OpenVINO 2025.x + NNCF | 一份代码跑 CPU/Intel GPU/NPU，量化无损 |
...
| 项 | 值 |
|---|---|
| 操作系统 | macOS（开发机）；目标部署 Intel Core Ultra 笔记本 |
| Python | 3.12 |
| OpenVINO | 2025.x |
...
### 5.4 性能基线（来源：项目调研 2500 用例 + 公开数据）

| 操作 | i5-6300U | Core Ultra (NPU) | Arc GPU |
|---|---|---|---|
| 文档解析 (100页 PDF) | 2.3s | 2.3s | 2.3s |
| BGE 向量构建 (1000 块) | 8.5s | 5.2s | 3.1s |
| RAG 语义检索 | 1.8s | 1.2s | 0.9s |
| LLM INT4 (7B) 生成 | 30 tok/s | 45 tok/s | 80 tok/s |

INT4 vs INT8 关键数据（500 用例实测）：

| 方案 | 大小 | 推理速度 | JSON 结构化输出成功率 |
|---|---|---|---|
| **INT4** | 4.2 GB | 30 tok/s | **100%** |
| INT8 | 7.6 GB | 8 tok/s | 33% |

**结论：AI PC Agent 场景 INT4 是唯一推荐**——既快又准。
...
while True:
    resp = ollama.chat(model="qwen3:30b-a3b-instruct",
...
只要本机起了 Ollama 并 `ollama pull qwen3:30b-a3b-instruct`，
...
# 2. 导出 OpenVINO INT4 模型（BGE ≈ 19MB，Qwen2.5-7B ≈ 4.2GB）
python export_models.py --model bge      # 必需
python export_models.py --model qwen     # 仅本地直接跑 LLM 时需要

# 3a. 链路烟雾测试（不依赖 Ollama，1 分钟跑完）
python examples/smoke_test.py

# 3b. 完整 Agent 体验（需要本机 Ollama）
ollama pull qwen3:30b-a3b-instruct
python run_agent.py "帮我读 my.pdf，里面有哪些风险条款？"
```
