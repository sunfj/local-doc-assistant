...
> 基于 RapidOCR + OpenVINO 的本地 OCR + RAG 智能问答 Skill，全程数据不出机。
...
- **本地 OCR 文字识别**：扫描件 PDF / 拍照图片 → RapidOCR（PP-OCRv4 ONNX 模型）本地识别，中英文置信度 98%+
...
- **硬件自适应**：`device=AUTO` 自动选择 NPU > GPU > CPU，无需手动配置
...
```bash
bash setup.sh   # 升级 pip → 配置镜像源 → 安装依赖 → 下载 BGE 模型 → 验证链路
```
...
导入 Skill 后，直接用自然语言对话即可：

> 帮我解析 examples/contract_scan.pdf，然后告诉我违约金比例是多少

QwenPaw 会自动按 SKILL.md 工作流依次调用 OCR → 索引 → 检索 → 回答。
...
| **PDF（扫描件）** | 自动检测 → RapidOCR | 无文字层的扫描件/照片 PDF |
| **图片** | RapidOCR 直接识别 | PNG / JPG / BMP / TIFF |
...
已在 QwenPaw 平台上完成端到端测试，扫描件合同 OCR → 问答全流程通过：

```
用户：帮我解析 examples/contract_scan.pdf，然后告诉我违约金比例是多少
→ 自动检测扫描件 → OCR 识别全文 → 建立索引 → 语义检索
→ 回答：逾期交付每日 0.5%，累计不超过 10%；拒收需额外赔偿 15%
```
...
- **OCR**：RapidOCR（PP-OCRv4 ONNX 模型，~30MB，无需 PaddlePaddle）
...
1. **扫描件自动检测 + OCR 路由**：`parse_document` 自动判断 PDF 是否有文字层，无文字层时无缝切换到 RapidOCR，用户无需关心底层差异
2. **合同照片 → 智能问答**：拍照/扫描的纸质文档，本地 OCR 识别 → 向量索引 → 自然语言问答，全流程数据不出机
3. **轻量级 OCR（~30MB）**：用 RapidOCR 替代 PaddlePaddle（500MB+），安装体积降低 94%，精度不变
4. **硬件自适应（AUTO）**：同一份代码自动适配 Mac（CPU）、Intel Core Ultra（NPU）、Arc GPU，零配置
5. **共享 Embedding 模块**：4 个 Tool 共享同一份 OpenVINO BGE 模型，避免重复加载（节省 800ms/次）
6. **INT4 + L2 归一化 + IndexFlatIP**：量化精度与语义检索在 FAISS 上一次性对齐
7. **框架无关**：manifest.json 转 OpenAI schema 后，Ollama / QwenPaw / LangChain 全部兼容
